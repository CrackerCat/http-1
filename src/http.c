...
static size_t header_readall(char *buffer, size_t size, size_t count,
...
struct http_body_t *http_body_init(void) {
  struct http_body_t *body =
      (struct http_body_t *)malloc(sizeof(struct http_body_t));
  if (body) {
    body->stream = NULL;
...
size_t http_body_readall(void *data, size_t size, size_t count,
                         struct http_body_t *body) {
...
struct http_request_t *http_request_init(void) {
  struct http_request_t *req =
      (struct http_request_t *)malloc(sizeof(struct http_request_t));
  if (req) {
    ...
    // follow any Location: header that the server sends as part of a HTTP
...
void http_add_header2(struct http_request_t *req, const char *name,
                      const char *value) {
  if (!req || !name || !value)
    return;
  if (strlen(name) > 0) {
...
void http_add_oauth2header(struct http_request_t *req,
                           char *(*get_access_token)(void),
                           void (*free_access_token)(char *token)) {
  if (req && get_access_token) {
    char *token = get_access_token();
    if (token) {
      char header[MAX_HEADER_LEN + 1] = {0};
      snprintf(header, MAX_HEADER_LEN, HTTP_HEADER_AUTHORIZATION ":Bearer %s",
               token);
      http_add_header(req, header);
...
struct http_response_t *http_response_init(void) {
  struct http_response_t *resp =
      (struct http_response_t *)malloc(sizeof(struct http_response_t));
  if (resp) {
    resp->status = 0L;
    resp->header = NULL;
    resp->body = NULL;
...
const char *http_post(struct http_request_t *req, const char *url,
  ...
  code = curl_easy_setopt(req->curl, CURLOPT_POSTFIELDSIZE,
  ...
  if (content_type) {
    ...
    snprintf(content_type_header, MAX_HEADER_LEN,
             HTTP_HEADER_CONTENTTYPE ": %s",
             ((!content_type) ? HTTP_CONTENTTYPE_STREAM : content_type));
  ...
  // effectively disables the sending of the specified string. libcurl will
...
const char *http_put(struct http_request_t *req, const char *url,
  ...
  char content_type_header[MAX_HEADER_LEN + 1] = {0};
  snprintf(content_type_header, MAX_HEADER_LEN, HTTP_HEADER_CONTENTTYPE ": %s",
           ((!content_type) ? HTTP_CONTENTTYPE_STREAM : content_type));
  http_add_header(req, content_type_header);

  if (req->body) {
    code = (req->body->len < (unsigned long long)2 * GB)
               ? curl_easy_setopt(req->curl, CURLOPT_INFILESIZE, req->body->len)
...
const char *http_do(struct http_request_t *req, struct http_response_t *resp) {
  ...
  if (resp->body) {
    ...
    code =
        curl_easy_setopt(req->curl, CURLOPT_WRITEFUNCTION, resp->body->fread);
    if (code != CURLE_OK)
      return curl_easy_strerror(code);
  }
  ...
  curl_easy_getinfo(req->curl, CURLINFO_NAMELOOKUP_TIME,
                    &(resp->namelookup_time));
  ...
  curl_easy_getinfo(req->curl, CURLINFO_SPEED_DOWNLOAD,
                    &(resp->download_speed));
